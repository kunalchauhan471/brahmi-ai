...
export default function LandingPage() {
  ...
  return (
    <div className="homepage min-h-screen bg-mesh overflow-hidden">
      ...
      {/* Navigation */}
      <motion.nav
        className="fixed top-0 left-0 right-0 z-50 nav-glass shadow-sm"
        initial={{ y: -80 }}
        animate={{ y: 0 }}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {steps.map((step, i) => (
              <motion.div
                key={i}
                className="relative overflow-hidden p-8 rounded-2xl bg-white border border-gray-100 shadow-card hover:shadow-card-hover gradient-ring transition-all duration-500 group"
                initial={{ opacity: 0, y: 40, scale: 0.95 }}
                whileInView={{ opacity: 1, y: 0, scale: 1 }}
                viewport={{ once: true, margin: '-60px' }}
                transition={{ delay: i * 0.15, duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
                whileHover={{ y: -10, scale: 1.03, boxShadow: '0 28px 56px -12px rgba(14, 165, 233, 0.22), 0 12px 24px -8px rgba(0, 0, 0, 0.1)' }}
              >
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, i) => (
              <motion.div
                key={feature.title}
                className="group relative p-6 rounded-2xl bg-white border border-gray-200/70 shadow-card hover:border-primary-200 transition-all duration-500 gradient-ring"
                initial={{ opacity: 0, y: 40, scale: 0.95 }}
                whileInView={{ opacity: 1, y: 0, scale: 1 }}
                viewport={{ once: true, margin: '-60px' }}
                transition={{ delay: i * 0.1, duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
                whileHover={{ y: -8, scale: 1.03, boxShadow: '0 24px 48px -12px rgba(14, 165, 233, 0.2), 0 10px 22px -8px rgba(0, 0, 0, 0.09)' }}
              >
